...
Element.collectTextNodes=function(a){return $A($P(a).childNodes).collect(function(b){return(b.nodeType==3?b.nodeValue:(b.hasChildNodes()?Element.collectTextNodes(b):""))
}).flatten().join("")
};
Element.collectTextNodesIgnoreClass=function(a,b){return $A($P(a).childNodes).collect(function(c){return(c.nodeType==3?c.nodeValue:((c.hasChildNodes()&&!Element.hasClassName(c,b))?Element.collectTextNodesIgnoreClass(c,b):""))
}).flatten().join("")
};
Element.setContentZoom=function(a,b){a=$P(a);
a.setStyle({fontSize:(b/100)+"em"});
if(Prototype.Browser.WebKit){window.scrollBy(0,0)
}return a
};
Element.getInlineOpacity=function(a){return $P(a).style.opacity||""
};
Element.forceRerendering=function(a){try{a=$P(a);
...
}else{e=$P(b).childNodes
}var a=Object.extend({speed:0.1,delay:0},arguments[2]||{});
var d=a.delay;
$A(e).each(function(g,f){new c(g,Object.extend(a,{delay:f*a.speed+d}))
})
},PAIRS:{slide:["SlideDown","SlideUp"],blind:["BlindDown","BlindUp"],appear:["Appear","Fade"]},toggle:function(b,c){b=$P(b);
...
Effect.Tween=Class.create(Effect.Base,{initialize:function(c,f,e){c=Object.isString(c)?$P(c):c;
...
Effect.Opacity=Class.create(Effect.Base,{initialize:function(b){this.element=$P(b);
...
Effect.Move=Class.create(Effect.Base,{initialize:function(b){this.element=$P(b);
if(!this.element){throw (Effect._elementDoesNotExistError)
...
Effect.Scale=Class.create(Effect.Base,{initialize:function(b,c){this.element=$P(b);
if(!this.element){throw (Effect._elementDoesNotExistError)
...
Effect.Highlight=Class.create(Effect.Base,{initialize:function(b){this.element=$P(b);
if(!this.element){throw (Effect._elementDoesNotExistError)
...
Effect.ScrollTo=function(d){var c=arguments[1]||{},b=document.viewport.getScrollOffsets(),e=$P(d).cumulativeOffset(),a=(window.height||document.body.scrollHeight)-document.viewport.getHeight();
if(c.offset){e[1]+=c.offset
}return new Effect.Tween(null,b.top,e[1]>a?a:e[1],c,function(f){scrollTo(b.left,f.round())
})
};
Effect.Fade=function(c){c=$P(c);
...
Effect.SlideDown=function(c){c=$P(c).cleanWhitespace();
...
Effect.SlideUp=function(c){c=$P(c).cleanWhitespace();
var a=c.down().getStyle("bottom");
var b=c.getDimensions();
...
Effect.Shrink=function(c){c=$P(c);
...
Effect.Pulsate=function(c){c=$P(c);
var b=arguments[1]||{};
var a=c.getInlineOpacity();
...
Effect.Morph=Class.create(Effect.Base,{initialize:function(c){this.element=$P(c);
if(!this.element){throw (Effect._elementDoesNotExistError)
...
if(document.defaultView&&document.defaultView.getComputedStyle){Element.getStyles=function(b){var a=document.defaultView.getComputedStyle($P(b),null);
return Element.CSS_PROPERTIES.inject({},function(c,d){c[d]=a[d];
return c
})
}
}else{Element.getStyles=function(b){b=$P(b);
...
}Effect.Methods={morph:function(a,b){a=$P(a);
new Effect.Morph(a,Object.extend({style:b},arguments[2]||{}));
return a
},visualEffect:function(c,e,b){c=$P(c);
var d=e.dasherize().camelize(),a=d.charAt(0).toUpperCase()+d.substring(1);
new Effect[a](c,b);
return c
},highlight:function(b,a){b=$P(b);
new Effect.Highlight(b,a);
return b
}};
$w("fade appear grow shrink fold blindUp blindDown slideUp slideDown pulsate shake puff squish switchOff dropOut").each(function(a){Effect.Methods[a]=function(c,b){c=$P(c);
