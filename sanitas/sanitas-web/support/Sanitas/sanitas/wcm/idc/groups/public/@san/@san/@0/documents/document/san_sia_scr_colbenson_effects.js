...
}var Droppables={drops:[],remove:function(a){this.drops=this.drops.reject(function(b){return b.element==$P(a)
})
},add:function(b){b=$P(b);
var a=Object.extend({greedy:true,hoverclass:null,tree:false},arguments[1]||{});
if(a.containment){a._containers=[];
var c=a.containment;
if(Object.isArray(c)){c.each(function(d){a._containers.push($P(d))
})
}else{a._containers.push($P(c))
...
}if(!this.handle){this.handle=$P(a.handle)
}if(!this.handle){this.handle=this.element
}if(a.scroll&&!a.scroll.scrollTo&&!a.scroll.outerHTML){a.scroll=$P(a.scroll);
...
Draggable._dragging={};
var SortableObserver=Class.create({initialize:function(b,a){this.element=$P(b);
this.observer=a;
...
}},options:function(a){a=Sortable._findRootElement($P(a));
if(!a){return
...
b.droppables.push(c)
}(b.elements||this.findElements(c,b)||[]).each(function(h,f){var g=b.handles?$P(b.handles[f]):(b.handle?$P(h).select("."+b.handle)[0]:h);
...
}if(!Sortable._marker){Sortable._marker=($P("dropmarker")||Element.extend(document.createElement("DIV"))).hide().addClassName("dropmarker").setStyle({position:"absolute"});
...
if(!a){continue
}var g={id:encodeURIComponent(a?a[1]:null),element:e,parent:f,children:[],position:f.children.length,container:$P(d[c]).down(b.treeTag)};
if(g.container){this._tree(g.container,b,g)
}f.children.push(g)
}return f
},tree:function(d){d=$P(d);
...
return a
},sequence:function(b){b=$P(b);
var a=Object.extend(this.options(b),arguments[1]||{});
return $P(this.findElements(b,a)||[]).map(function(c){return c.id.match(a.format)?c.id.match(a.format)[1]:""
})
},setSequence:function(b,c){b=$P(b);
