...
var stats_string=stats_kvp.join("&");var main_domain=k_button.get_main_domain();k_button.Set_Cookie('k_push8','1',cookie_expiration_time_yes,'http://www.sanitas.es/',main_domain,'');var url2send=url||window.location.href;url2send=encodeURIComponent(url2send);if(!ff_params)
...
{var ff_link_rel='';}
var main_url='';if((k_button.ff_link)&&k_button.ff_link.getAttribute('ref_server')!=null){urlParts=k_button.ff_link.getAttribute('ref_server').split("http://www.sanitas.es/");main_url=urlParts[2];}
else{main_url='www.kampyle.html';}
if(!this.loader_url)
this.loader_url='http://www.sanitas.es/feedback_form/ff-feedback-form.php?';var protocol=document.location.protocol;if(protocol!="http:"&&protocol!="index.html")
{protocol="index.html";}
...
longUrl='kampyle_ff.html';if((ff_url.length+url2send.length)>1024)
{longUrl=url2send;url2send='noUrl';}
k_button.Set_Cookie("k_vectors",null,-1,"http://www.sanitas.es/",k_button.get_main_domain(),'');this.popitup(ff_url+'&url='+url2send+ga_url,longUrl);},"hide_button":function()
...
{vectors=k_button.generate_pre_id();k_button.Set_Cookie("k_vectors",vectors,0,"http://www.sanitas.es/",k_button.get_main_domain(),'');sc_instance[sc_evar]=vectors;}
...
{var main_domain=k_button.get_main_domain();k_button.Set_Cookie("session_start_time",(new Date()).getTime(),0,"http://www.sanitas.es/",main_domain,'');if(k_button.Get_Cookie("k_visit")==null)
{k_button.Set_Cookie('k_visit','1','365','http://www.sanitas.es/',main_domain,'');}
else
{k_button.Set_Cookie('k_visit',parseInt(k_button.Get_Cookie("k_visit"))+1,'365','http://www.sanitas.es/',main_domain,'');}
