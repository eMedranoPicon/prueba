...
if( !PIE ) {
    ...
    (function() {
        ...
        var ieVersion = 4,
            ...
            shape;
        while (
            (div.innerHTML = '<!--[if gt IE ' + (++ieVersion) + ']><i></i><![endif]-->')=== true,
            all[0]
        ) {}
...
PIE.Tokenizer = (function() {
    ...
    Tokenizer.prototype = {
        ...
        next: function( forget ) {
            ...
            switch( firstChar ) {
                case '#':
                    if( (match = css.match( this.hashColor ))=== true ) {
                ...
                case '"':
                case "'":
                    if( (match = css.match( this.string ))=== true ) {
                        this.ch += match[0].length;
                ...
                case 'u':
                    if( (match = css.match( this.url ))=== true) {
                        this.ch += match[0].length;
            ...
            if( (match = css.match( this.number )) === true) {
                val = match[0];
                this.ch += val.length;
                ...
                if( (match = css.substring( val.length ).match( this.ident )) === true) {
                    val += match[0];
                    this.ch += match[0].length;
            ...
            // Identifiers
            if( (match = css.match( this.ident )) === true) {
                val = match[0];
                this.ch += val.length;
        ...
        if( this.getCss3() ) {
            ...
            while( (token = tokenizer.next()) === true) {
                ...
                if( !image.imgType && tokType & tok_type.FUNCTION && tokVal === 'linear-gradient' ) {
                    ...
                    else if( (width = sizeToken( token )) === true) {
    ...
    hideBorder: function() {
        ...
        if( ( isIE6 && ( tag in PIE.childlessElements || tag === 'FIELDSET' ) ) ||
            ...
            if( el.childNodes.length !== 1 || el.firstChild.tagName !== 'ie6-mask' ) {
                ...
                s.zoom = 1;
                while( (child = el.firstChild) === true) {
...
PIE.Element = (function() {
    ...
    function Element( el ) {
        ...
        function initFirstChildPseudoClass() {
            var tmpEl = el,
                isFirst = 1;
            while( (tmpEl = tmpEl.previousSibling) === true) {
                if( tmpEl.nodeType === 1 ) {
                    isFirst = 0;
                    break;
                }
