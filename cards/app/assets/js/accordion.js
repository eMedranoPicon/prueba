( function ( $ ) {
  ...
  $.AccordionSoft.defaults = {
    ...
    enableScroll: true,
    classOpenned: 'js-openned',
    selectorContent: '.month-view__content',
    selectorHeaderContainer: '.month-view__heading',
    selectorHeader: '.month-view__heading span'
  };
  ...
  $.AccordionSoft.prototype = {
    _init: function( options ) {
      ...
      this.$items.find( this.options.selectorContent ).hide();
    ...
    _firstItem: function( $item ) {
      var $content = $item.find( this.options.selectorContent );
      var $header = $item.find( this.options.selectorHeader );

      ( $item.hasClass( this.options.classOpenned ) )

      ?
      ( this.current = -1,
        $content.stop( true, true ).fadeOut( this.options.speed ),
        $header.stop( true, true ).fadeIn( this.options.speed ),
        $item.removeClass( this.options.classOpenned ).stop().animate( {
      ...
      ( this.current = $item.index(),
        ...
        $item.addClass( this.options.classOpenned ).stop().animate( {
    ...
    _isOpened: function() {
      return ( this.$el.find( 'li.' + this.options.classOpenned ).length > 0 );
    },
    ...
    _saveDimValues: function() {
      ...
      this.$items.each( function() {
        ...
        $item.data( {
          originalHeight: $item.find( instance.options.selectorHeaderContainer ).height(),
    ...
    _initEvents: function() {
      ...
      // open / close item
      this.$items.find( this.options.selectorHeaderContainer ).bind( 'click.accordion', function() {
      ...
      $( window ).bind( 'smartresize.accordion', function() {
        ...
        instance.$el.find( 'li' + this.options.classOpenned ).each( function() {
          var $this = $( this );
          $this.css( 'height', $this.data( 'originalHeight' ) + $this.find( instance.options.selectorContent ).outerHeight( true ) );
        } );
    ...
    _scroll: function( instanceArg ) {
      ...
      var instance = instanceArg || this,
        current;

      ( instance.current !== -1 ) ? current = instance.current: current = instance.$el.find( 'li.' + this.options.classOpenned +':last' ).index();
