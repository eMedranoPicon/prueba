var appFront = angular.module("appFront", [ 'checkImg', 'acronimoPais',
		'acronimoLugar', 'mesEnLiteral', 'ui.bootstrap', 'ui.map', 'ui.event',
		'titlePage', 'guthub.directives' ]);
...
appFront
		.config([
				...
				function($routeProvider, $httpProvider) {
					...
					PlaceDetailController.$inject = [ '$scope', '$http',
							'$routeParams', '$rootScope', '$location',
							'mySharedServicePlace' ];
					PlaceMapController.$inject = [ '$scope', '$rootScope',
							'mySharedServicePlace' ];

				} ]);

appFront.factory('mySharedService',
		function($rootScope) {
			var sharedService = {};
			...
			sharedService.prepForBroadcast = function(lat, lon, calle, cp,
					ciudad, pais) {
				this.latitud = lat;
				this.longitud = lon;
				this.calleBdc = calle;
...
appFront.factory('mySharedServicePlace', function($rootScope) {
	var sharedServicePlace = {};

	sharedServicePlace.latitude = '';
	sharedServicePlace.longitud = '';

	sharedServicePlace.prepForBroadcastPlace = function(lat, lon) {
		this.latitude = lat;
		this.longitud = lon;
		this.broadcastItem();
	};
	sharedServicePlace.broadcastItem = function() {
		$rootScope.$broadcast('handleBroadcastPlace');
	};
	return sharedServicePlace;
});
